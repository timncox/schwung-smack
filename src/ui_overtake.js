 ...
 *                         72 Clear / 73 Fill (tap latches, hold is
 *                         momentary); hardware Capture button = retro grab
 *   Knobs 1-8             FX Density, Order, Loop Len, Slice Res,
 *                         Wet, Pitch Range, Fill Amt, Seed
 *                         (AB Quantize moved to the hierarchy Setup page)
...
function paintTransport(force) {
    ...
    setLED(PAD_FILL, state === 3 ? (fill ? White : YellowGreen) : Black, force);
    for (let p = 74; p <= 75; p++) setLED(p, Black, force);
}
...
function paintSteps(force) {
    ...
    for (let i = 0; i < STEP_COUNT; i++) {
        ...
        if (state === 3 && i < shown) {
            if (fill) {
                /* the fill layer sounds over either side, so it shows
                 * itself whenever it is active */
                const code = fillPattern.charCodeAt(i) - 48;
...
globalThis.tick = function() {
    ...
    if (tickCount % 12 === 0) {
        ...
        /* speak async transitions (armed -> recording -> looping); A/B and
         * Fill are announced at press time (applied value lags a quantized
         * flip) */
        if (state !== oldState)
